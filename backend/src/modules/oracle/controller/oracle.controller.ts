...
export class OracleController {
  ...
  async getViralityScore(@Body() request: OracleRequestDto): Promise<OracleResponseDto> {
    this.logger.log(`📨 Received virality request for trend: ${request.trendId}`);

    try {
      const result = await this.oracleCoordinator.processOracleRequest(request);
      this.logger.log(`✅ Virality score generated: ${result.viralityScore.toFixed(4)} for trend: ${request.trendId}`);
      return result;
    } catch (error) {
  ...
  async getLatestViralityScore(@Param('trendId') trendId: string): Promise<OracleResponseDto> {
    this.logger.log(`📨 Requesting latest virality score for trend: ${trendId}`);
  ...
  async verifyProof(@Param('proofHash') proofHash: string): Promise<any> {
    ...
    this.logger.log(`✅ Proof verification completed: ${result.verified ? 'VALID' : 'INVALID'}`);
  ...
  async getOracleStatus(): Promise<any> {
    this.logger.log('📊 Requesting Oracle network status');
    ...
    this.logger.log(`✅ Oracle status retrieved - Network: ${status.networkType}, Validators: ${status.validators.healthyValidators}/${status.validators.totalValidators}, Phase 2: ${enhancedStatus.phase2.enabled ? 'ENABLED' : 'DISABLED'}`);
  ...
  async getTrendingTikTokVideos(@Query('limit') limit?: number): Promise<SocialMediaPost[]> {
    ...
    this.logger.log(`📱 Requesting ${videoLimit} trending TikTok videos`);

    try {
      const videos = await this.realSocialDataService.getTrendingTikTokVideos(videoLimit);
      this.logger.log(`✅ Retrieved ${videos.length} TikTok videos`);
      return videos;
    } catch (error) {
  ...
  async getTrendingTwitterTopics(@Query('limit') limit?: number): Promise<SocialMediaPost[]> {
    ...
    this.logger.log(`🐦 Requesting ${topicLimit} trending Twitter topics`);

    try {
      const topics = await this.realSocialDataService.getTrendingTwitterTopics(topicLimit);
      this.logger.log(`✅ Retrieved ${topics.length} Twitter topics`);
      return topics;
    } catch (error) {
  ...
  async getTrendingInstagramPosts(@Query('limit') limit?: number): Promise<SocialMediaPost[]> {
    ...
    this.logger.log(`📷 Requesting ${postLimit} trending Instagram posts`);

    try {
      const posts = await this.realSocialDataService.getTrendingInstagramPosts(postLimit);
      this.logger.log(`✅ Retrieved ${posts.length} Instagram posts`);
      return posts;
    } catch (error) {
  ...
  async getTrendingYouTubeVideos(@Query('limit') limit?: number): Promise<SocialMediaPost[]> {
    ...
    this.logger.log(`📺 Requesting ${videoLimit} trending YouTube videos`);

    try {
      const videos = await this.realSocialDataService.getTrendingYouTubeVideos(videoLimit);
      this.logger.log(`✅ Retrieved ${videos.length} YouTube videos`);
      return videos;
    } catch (error) {
  ...
  async getTrendingFacebookPosts(@Query('limit') limit?: number): Promise<SocialMediaPost[]> {
    const postLimit = limit || 50;
    this.logger.log(`📘 Requesting ${postLimit} trending Facebook posts`);

    try {
      const posts = await this.realSocialDataService.getTrendingFacebookPosts(postLimit);
      this.logger.log(`✅ Retrieved ${posts.length} Facebook posts`);
      return posts;
    } catch (error) {
  ...
  async getSouthAfricanTrends(): Promise<SocialMediaPost[]> {
    this.logger.log('🇿🇦 Requesting aggregated South African trends');

    try {
      const trends = await this.realSocialDataService.getSouthAfricanTrends();
      this.logger.log(`✅ Retrieved ${trends.length} South African trends`);
      return trends;
    } catch (error) {
  ...
  async classifyTrend(@Body() posts: SocialMediaPost[]): Promise<any> {
    ...
    try {
      ...
      this.logger.log(`✅ Trend classified as: ${classification.category} with confidence: ${classification.confidence}`);
  ...
  async getClassificationByTrendId(@Param('trendId') trendId: string): Promise<any> {
    this.logger.log(`🔍 Requesting classification for trend: ${trendId}`);
  ...
  async detectDeception(@Body() posts: SocialMediaPost[]): Promise<any> {
    ...
    try {
      ...
      this.logger.log(`✅ Deception analysis completed: ${deceptionResult.isDeceptive ? 'DECEPTIVE' : 'AUTHENTIC'} (confidence: ${deceptionResult.confidence})`);
  ...
  async getOracleHistory(
    ...
    this.logger.log(`✅ Retrieved ${history.length} historical records for trend: ${trendId}`);
  ...
  async getMetrics(): Promise<any> {
    this.logger.log('📊 Requesting Oracle metrics');
