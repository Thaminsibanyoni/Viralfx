...
registry.registerMetric(this.settlementDuration);
  ...
  private async settleBet(job: Job<SettleBetJob>): Promise<void> {
    try {
  ...
  private async settleMarket(job: Job<SettleMarketJob>): Promise<void> {
    try {
  ...
  private async settleOrder(job: Job<SettleOrderJob>): Promise<void> {
    ...
    try {
      ...
      const order = await this.prisma.order.findFirst({ where: { id: orderId }, relations: ['user'] });
  ...
  private async bulkSettleBets(job: Job<{ marketId: string }>): Promise<void> {
    try {
